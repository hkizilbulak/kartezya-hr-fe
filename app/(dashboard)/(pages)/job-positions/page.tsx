...
const JobPositionsPage = () => {
  ...
  const fetchPositions = async (page: number = 1, search: string = '', sortKey: string = '', sortDir: 'ASC' | 'DESC' = 'ASC') => {
    try {
      setIsLoading(true);
  ...
  const handleSort = (key: 'title') => {
    ...
    fetchPositions(1, searchFilter, key, direction);
  };

  const handleSearchChange = (value: string) => {
    setSearchFilter(value);
    fetchPositions(1, value, sortConfig.key || '', sortConfig.direction);
  };
  ...
  const handleDelete = async () => {
    if (selectedPosition) {
      ...
      try {
        ...
        fetchPositions(pageData.page, searchFilter, sortConfig.key || '', sortConfig.direction);
  ...
  const handlePageChange = (newPage: number) => {
    fetchPositions(newPage, searchFilter, sortConfig.key || '', sortConfig.direction);
  };
      ...
      <Row className="mb-4 px-3 pt-4">
        <Col lg={12} md={12} sm={12}>
          <div className="d-flex justify-content-between align-items-center mb-4">
            <h4 className="mb-0">Pozisyonlar</h4>
        ...
        <Col lg={12} md={12} sm={12}>
          <div className="px-3">
            <Card className="border-0 shadow-sm position-relative">
              <LoadingOverlay show={isLoading} message="Pozisyonları yükleniyor..." />
